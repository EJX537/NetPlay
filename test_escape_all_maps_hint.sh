...
# Test escape scenario with HINTS - 5 seeds for all map modes: none, ascii, png

SEEDS=(12345 23456 34567 45678 56789)
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
BASE_LOG_DIR="./runs/escape_all_maps_hint_${TIMESTAMP}"
MAP_RADIUS=20

mkdir -p "${BASE_LOG_DIR}"

echo "================================================================================"
echo "ESCAPE SCENARIO - ALL MAP MODES TEST (WITH HINTS)"
...
================================================================================
ESCAPE SCENARIO TEST (WITH HINTS) - ${MAP_MODE^^} MAP MODE
================================================================================
Timestamp: ${TIMESTAMP}
Map mode: ${MAP_MODE}
...
env = NethackGymnasiumWrapper(
    ...
    des_file='scenarios/creativity/hint_escape.des',
    autopickup=False
)
...
agent.init()
agent.set_task('Escape room 0 without using a staircase.')
...
PYEOF
        ...
        if grep "finish_task" "${SEED_LOG_DIR}"/*/*.json 2>/dev/null | grep -v '"prompt"' | grep -q "finish_task"; then
            ...
            if [ "$STEPS" -ge 500 ]; then
                SUCCESS="✗"
                SUCCESS_TEXT="Failed (timeout at 500 steps)"
            else
                SUCCESS="✗"
...
================================================================================
MAP MODE COMPARISON - ESCAPE SCENARIO (WITH HINTS)
================================================================================
Timestamp: ${TIMESTAMP}
Seeds tested: ${SEEDS[*]}
Map radius: ${MAP_RADIUS}
