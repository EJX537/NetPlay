...
# Test carry scenario with 5 seeds for all map modes - GUIDED version with tool instructions

SEEDS=(12345 23456 34567 45678 56789)
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
BASE_LOG_DIR="./runs/carry_all_maps_guided_${TIMESTAMP}"
MAP_RADIUS=20

mkdir -p "${BASE_LOG_DIR}"

echo "================================================================================"
echo "CARRY SCENARIO - ALL MAP MODES TEST (GUIDED)"
...
================================================================================
CARRY SCENARIO TEST - ${MAP_MODE^^} MAP MODE (GUIDED)
================================================================================
Timestamp: ${TIMESTAMP}
Map mode: ${MAP_MODE}
...
env = NethackGymnasiumWrapper(
    ...
    des_file='scenarios/creativity/carry.des',
    autopickup=False
)
...
agent.init()
agent.set_task('Drop a plate mail and a iron ball in the room containing the statue. Make sure to first defeat any monster that you encounter. The items are very heavy, only pickup one at a time and use a teleportation wand to teleport yourself with the item.')
...
PYEOF
        ...
        if grep "finish_task" "${SEED_LOG_DIR}"/*/*.json 2>/dev/null | grep -v '"prompt"' | grep -q "finish_task"; then
            ...
            if [ "$STEPS" -ge 100 ]; then
                SUCCESS="✗"
                SUCCESS_TEXT="Failed (timeout at 100 steps)"
            else
                SUCCESS="✗"
...
================================================================================
MAP MODE COMPARISON - CARRY SCENARIO (GUIDED)
================================================================================
Timestamp: ${TIMESTAMP}
Seeds tested: ${SEEDS[*]}
Map radius: ${MAP_RADIUS}
