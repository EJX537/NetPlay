...
from autoascend.nethack_monitor import NethackH5PYMonitor


def prepare_env(args, seed, log_folder):
    seed += args.seed
    ...
    visualizer_args = dict(enable=args.mode == 'run' or visualize_with_simulate,
                           ...
                           output_video_path=os.path.join(args.log_folder, f'video.mp4'))

    env = gym.make('NetHackChallenge-v0', no_progress_timeout=1000)
...
def single_simulation(args, seed_offset, timeout=720):
    ...
    simulation_folder = os.path.join(args.log_folder, f"seed_{seed_offset}")
    if os.path.exists(simulation_folder):
        raise Exception(f"Simulation folder '{simulation_folder}' already exists.")
    os.makedirs(simulation_folder)

    env = prepare_env(args, seed_offset, simulation_folder)
    try:
...
def parse_args():
    ...
    parser.add_argument('--log_folder', type=str, default="./runs", help='The folder where all the logs will be stored.')
    ...
    args.log_folder = os.path.join(args.log_folder, datetime.datetime.now().strftime("runs_%Y-%m-%d--%H%M%S"))
